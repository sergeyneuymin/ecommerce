...
export class CheckoutComponent implements OnInit {
  ...
  setupStripePaymentForm() {
    ...
    // create a card element and hide the zip-code fields
    this.cardElement = elements.create('card', { hidePostalCode: true });

    // add an instance of card UI component into the 'card-element' div block
  ...
  onSubmit() {
    ...
    // if valid form then create payment intent, confirm card payment and place an order
            ...
            .then((result: any) => {
              if(result.error) {
                ...
                this.checkoutService.placeOrder(purchase).subscribe({
                  ...
                  error: (err: any) => {
                    alert(`There was an error: ${err.message0}`);
                    this.isDisabled = false;
                  }
              })
              }
